    ...
    <main className="page" id="dashboard-page">
      <div className="container">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 28, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <button className="btn btn-secondary btn-sm" onClick={() => navigate('/')} id="btn-back">← Back</button>
              <span style={{
            ...
            <p className="page-subtitle">{job.company_name} · {job.decay_window_hours}h acknowledge window</p>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {process.env.NODE_ENV !== 'production' && (
              <button className="btn btn-secondary btn-sm" onClick={handleTriggerDecay} id="btn-decay" title="Dev: Trigger decay cycle now">
                ⚡ Decay
              </button>
            )}
        ...
        <div className="stats-grid" style={{ marginBottom: 20 }}>
          ...
          <div className="stat-card">
            <div className="stat-value">{stats.total_decays ?? 0}</div>
          ...
          <div id="pipeline-tab">
            ...
            {active.length === 0 && (
              <div className="empty-state">
                ...
                <div className="empty-state-text">No active applicants. Open applications on the home page.</div>
              </div>
            )}
                  ...
                  <tbody>
                    {waitlist.map(app => (
                      <tr key={app.id} id={`waitlist-row-${app.id}`}>
                        ...
                        <td style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{app.email}</td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setConfirmExit(null)}>
          <div className="modal" role="dialog" aria-modal="true" aria-labelledby="confirm-exit-title">
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>
              Remove <strong>{confirmExit.name}</strong> from the pipeline? The next waitlisted applicant will be promoted automatically.
            </p>
            <div className="modal-actions">
              ...
              <button
                ...
                disabled={actionLoading}
              >
                {actionLoading ? <><span className="spinner" style={{ width: 14, height: 14 }} /> Removing…</> : 'Yes, Reject'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCapacityModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowCapacityModal(false)}>
          <div className="modal" role="dialog" aria-modal="true" aria-labelledby="capacity-modal-title">
            <h2 className="modal-title" id="capacity-modal-title">Update Active Capacity</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginBottom: 20 }}>
              Current: <strong>{pipeline.capacity_total}</strong>. Increasing will immediately promote from waitlist; decreasing will demote excess active applicants.
            </p>
            <form onSubmit={handleCapacityChange}>
              <div className="form-group" style={{ marginBottom: 16 }}>
                <label className="form-label" htmlFor="new-capacity">New Capacity</label>
                <input
                  id="new-capacity"
                  type="number"
                  min="1"
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn btn-primary" id="btn-capacity-confirm" disabled={actionLoading}>
